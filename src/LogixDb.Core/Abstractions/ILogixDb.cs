...
public interface ILogixDb
{
    ...
    /// <param name="token">A cancellation token to cancel the operation.</param>
    /// <returns>A task that represents the asynchronous operation, containing the added snapshot with updated metadata.</returns>
    ...
    /// <returns></returns>
    Task UpsertSnapshot(Snapshot snapshot, CancellationToken token = default);
