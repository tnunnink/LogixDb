MERGE INTO logix.program AS target
...
SELECT @VersionId,
       (SELECT program_id FROM logix.program WHERE record_hash = t.record_hash),
       (SELECT component_id FROM logix.target_component WHERE component_name = 'program')
FROM #temp_program t;
