CREATE PROCEDURE [qa].[get_variable_as_bit]
...
BEGIN
    ...
    SELECT @variable = TRY_CAST(@value AS BIT);

    IF @variable IS NULL
        BEGIN
            SET @message = FORMATMESSAGE('''%s'' could not be parsed as bit for key ''%s''' ,@value, @key);
            THROW 50000, @message, 1;
        END
END;
GO
