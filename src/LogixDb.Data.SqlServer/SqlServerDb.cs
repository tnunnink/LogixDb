...
public sealed class SqlServerDb(DbConnectionInfo connection) : ILogixDb
{
    ...
    public async Task Purge(CancellationToken token = default)
    {
        ...
        await ExecuteSqlAsync(SqlCode.DeleteTargets, token: token);
    }
    ...
    public async Task<IEnumerable<Snapshot>> ListSnapshots(string? targetKey = null, CancellationToken token = default)
    {
        ...
        return await connection.QueryAsync<Snapshot>(SqlCode.ListSnapshots, key);
    }
    ...
    public async Task<Snapshot> GetSnapshot(string targetKey, int version = 0, CancellationToken token = default)
    {
        ...
        return await connection.QuerySingleAsync<Snapshot>(SqlCode.GetLatestSnapshot, new { target_key = targetKey });
    }
    ...
    public async Task DeleteSnapshot(string targetKey, int version, CancellationToken token = default)
    {
        ...
        await ExecuteSqlAsync(SqlCode.DeleteSnapshotByVersion, param, token);
    }
        ...
        CancellationToken token = default)
    {
        ...
        await ExecuteSqlAsync(SqlCode.DeleteSnapshotsBefore, param, token);
    }
    ...
    private static async Task CreateSnapshotAsync(SqlDbSession session, Snapshot snapshot)
    {
        ...
        var targetId = await session.GetAsync<Guid>(SqlCode.GetTargetId, new { target_key = snapshot.TargetKey });

        // Retrieve the target key back from the database since it could already exist.
        var versionNumber = await session.GetAsync<int>(SqlCode.GetLatestVersion, new { target_id = targetId });
        ...
        await session.ExecuteAsync(SqlCode.InsertSnapshotMetadata,
    ...
    {
        var key = new { target_key = targetKey };
        var snapshots = await session.GetAllAsync<Snapshot>(SqlCode.ListSnapshots, key);
    ...
    /// </summary>
    private async Task EnsureDatabase(CancellationToken token = default)
    {
        try
        {
